...
${req.body.documents}
 ...
 `;
    ...
    var immiText = `
    <strong>ثبت پروژه جدید</strong>
    ${req.body.contact} درخواست جدید ارسال کرده 
    فایل ترجمه :
    <a href="http://onita.ir/users-docs/"${req.body.doc}>${req.body.doc}</a>
    `;

  if (req.body.immi) {
    var propText = immiText ;
  }
    

    bot.sendMessage(ADMIN_BOT, propText, {parse_mode: 'HTML'});
    // res.send(`http://198.143.181.55:1212/download/${req.body.document}`)
  
})

app.post('/v1/partnershipReq', function(req,res,next){
  var expert = `
    <strong> درخواست همکاری ${req.body.expName} </strong>
    رزومه : ${req.body.expResume}
    تلگرام : ${req.body.expTelegram}
    ایمیل : ${req.body.expEmail}
    شماره تماس : ${req.body.expTel}
    امکان صحبت : ${req.body.expVoiceCall}
  `;

  bot.sendMessage(ADMIN_BOT, expert, {parse_mode: 'HTML'}).then(function(data){
    if (data.message_id){
    res.status(200).send({data: 'EO'});
  } else {
    res.status(404).send({err: 'SFIMoNW'});
  }
});
...
app.post('/v1/uploadDocs' , function(req, res, next) {
  var form = new formidable.IncomingForm();
  var docAddress, fileName;
  form.uploadDir = './users-docs';
  form.keepExtensions = true;
